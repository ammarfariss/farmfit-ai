...
export default function Home() {
  ...
  return <main className="app-shell">
    ...
    {step > 0 && <div className="workspace"><aside className="sidebar"><div className="step-label">FARM PLAN / 0{Math.min(step, 3)}</div><h2>{step === 3 ? "Your farm blueprint" : "Build your plan"}</h2>{step < 3 && <div className="stepper">{["Select land", "Choose what you can use", "Set limits"].map((label, index) => <button key={label} className={step === index + 1 ? "active" : step > index + 1 ? "complete" : ""} onClick={() => setStep(index + 1)}><span>{index + 1}</span>{label}</button>)}</div>}
      ...
      {result && <ResultsPanel result={result} siteData={siteData} waterLimitM3={water} energyLimitKwh={energy} onReset={() => { setResult(null); setSiteData(null); setStep(1); }} />}
...
function ResultsPanel({ result, siteData, waterLimitM3, energyLimitKwh, onReset }: { result: PortfolioResult; siteData: { source: string; climate: { temperatureC: number | null; solarKwhM2Day: number | null; humidityPct: number | null; windMps: number | null }; nearby?: { markets: number; roads: number }; soil?: { pH: number | null; source: string }; qatarDatasets?: Array<{ id: string; title: string }> } | null; waterLimitM3: number; energyLimitKwh: number; onReset: () => void }) {
  const resourceData = [
    { name: "Water", usedPct: waterLimitM3 > 0 ? Math.round(result.waterM3 / waterLimitM3 * 100) : 0 },
    { name: "Energy", usedPct: energyLimitKwh > 0 ? Math.round(result.energyKwh / energyLimitKwh * 100) : 0 },
  ];
  ...
  return <div className="results"><div className="result-head"><p className="section-kicker">OPTIMIZED PORTFOLIO</p><h3>Built around your land.</h3><button className="text-button" onClick={onReset}>Edit plan</button></div><div className="metric-hero"><span>Scenario 5-year ROI</span><strong>{Math.round(result.roi5Year * 100)}%</strong><small>Payback in {Number.isFinite(result.paybackYears) ? `${result.paybackYears.toFixed(1)} years` : "not reached"}</small></div><div className="metric-grid">{[["Annual revenue", money(result.annualRevenueQar)], ["Annual profit", money(result.annualProfitQar)], ["CapEx", money(result.capexQar)], ["Annual OpEx", money(result.annualOpexQar)], ["Water", `${Math.round(result.waterM3).toLocaleString()} m³`], ["Energy", `${Math.round(result.energyKwh).toLocaleString()} kWh`], ["Site fit", result.siteFitPct === null ? "Baseline" : `${Math.round(result.siteFitPct)}%`]].map(([label, value]) => <div className="metric" key={label}><span>{label}</span><strong>{value}</strong></div>)}</div><div className="visual-card"><div className="subhead"><h4>Resource envelope</h4><span>% of your selected limit</span></div><ResponsiveContainer width="100%" height={120}><BarChart data={resourceData} layout="vertical" margin={{ top: 0, right: 12, left: 8, bottom: 0 }}><CartesianGrid strokeDasharray="3 3" horizontal={false} stroke="#e5ebe6" /><XAxis type="number" domain={[0, 100]} hide /><YAxis dataKey="name" type="category" width={48} tick={{ fontSize: 10, fill: "#6f7d77" }} /><Tooltip formatter={(value) => [`${value}%`, "used"]} /><Bar dataKey="usedPct" fill="#2d5b52" radius={[0, 5, 5, 0]} /></BarChart></ResponsiveContainer></div><div className="site-card"><div><span className="section-kicker">SITE CONDITIONS</span><h4>Climate + soil snapshot</h4></div>{siteData ? <><small>{siteData.source}</small><div className="site-grid"><span><b>{siteData.climate.temperatureC?.toFixed(1) ?? "—"}°C</b>temperature</span><span><b>{siteData.climate.solarKwhM2Day?.toFixed(1) ?? "—"}</b>kWh/m²/day solar</span><span><b>{siteData.climate.humidityPct?.toFixed(0) ?? "—"}%</b>humidity</span><span><b>{siteData.climate.windMps?.toFixed(1) ?? "—"} m/s</b>wind</span><span><b>{siteData.soil?.pH?.toFixed(1) ?? "—"}</b>soil pH</span><span><b>{siteData.nearby?.markets ?? "—"}</b>nearby markets</span><span><b>{siteData.nearby?.roads ?? "—"}</b>roads in 3 km</span><span><b>{siteData.qatarDatasets?.length ?? "—"}</b>Qatar datasets found</span></div></> : <small>Loading NASA POWER, SoilGrids, Qatar Open Data, and OpenStreetMap context…</small>}</div><div className="portfolio"><div className="subhead"><h4>Portfolio</h4><span>{Math.round(result.totalAreaM2).toLocaleString()} / {Math.round(result.availableAreaM2).toLocaleString()} m² allocated</span></div>{result.allocations.map((item) => <div className="allocation" key={`${item.cropId}-${item.techniqueId}`}><span className="allocation-bar" style={{ width: `${item.percentage * 100}%`, background: CROPS.find((crop) => crop.id === item.cropId)?.color }} /><div><strong>{CROPS.find((crop) => crop.id === item.cropId)?.name}</strong><small>{TECHNIQUES.find((technique) => technique.id === item.techniqueId)?.name}</small></div><b>{Math.round(item.percentage * 100)}%</b></div>)}{unallocatedPct > 0.01 && <div className="allocation"><span className="allocation-bar" style={{ width: `${unallocatedPct * 100}%`, background: "#d7ddd9" }} /><div><strong>Unallocated reserve</strong><small>Left unused because a resource or budget limit is binding</small></div><b>{Math.round(unallocatedPct * 100)}%</b></div>}</div><div className="why"><h4>Why this layout?</h4>{result.explanations.slice(0, 4).map((explanation) => <p key={explanation}><span>↗</span>{explanation}</p>)}</div></div>;
}
